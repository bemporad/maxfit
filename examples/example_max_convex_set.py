# Find convex inner approximation of a nonconvex set using the maxfit algorithm. The nonconvex function is defined as a Python function, and the convex inner approximation is defined as a JAX function with learnable parameters.
#
# {x: fhat(x) + err <=0} \subseteq {x: f(x)<=0} 
#
# (C) 2026 A. Bemporad
...
# ################################
# Neural network approximation architecture
n1 = 8
n2 = 8
act = jax.nn.softplus
@jax.jit
def output_fcn_(u, params):
    V1,b1,V2,W2,b2,V3,W3,b3 = params
    y = act(V1@u.reshape(nx,1))+b1
    y = act((W2**2)@y)+V2@u.reshape(nx,1)+b2
    y = (W3**2)@y+V3@u.reshape(nx,1)+b3 #+jnp.sum((C@u)**2)+d.T@u+e
    return y.reshape(-1)
def init_fcn(seed):
    np.random.seed(seed)
    return [np.random.randn(n1,nx), np.random.randn(n1,1),
            np.random.randn(n2,nx), np.random.randn(n2,n1), np.random.randn(n2,1),
            np.random.randn(1,nx), np.random.randn(1,n2), np.random.randn(1,1)] 
            #np.random.randn(nx,nx), np.random.randn(nx,1), np.random.rand(1, 1)]
...
print(f"err = {-results.worst_error:.5f}")
print("done.")
