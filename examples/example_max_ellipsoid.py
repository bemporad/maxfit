# Find inner ellipsoidal approximation of a nonconvex set using the maxfit algorithm. The nonconvex function is defined as a Python function, and the convex inner approximation is defined as a JAX function with learnable parameters.
#
# {x: (x-x0)'P(x-x0) <= 1 - err} \subseteq {x: f(x)<=0} 
#
# (C) 2026 A. Bemporad
...
warm_retrain = True  # set to True to retrain the model at each iteration starting from previous model parameters to speed up training, although this may lead to worse results in some cases. 
...
# Ellipsoid
# ##########
# Parameterize P = C'*C, which is always positive semidefinite. Matrix C is lower-triangular to avoid redundant variables.
# The optimization vector is [x0, c], where x0 is the center of the ellipsoid and c are the lower-triangular entries of C,
# which has length nx*(nx+1)/2.
_tril_rows, _tril_cols = np.tril_indices(nx)

@jax.jit
def output_fcn_(u, params):
    x0, c = params
    v = u - x0.reshape(-1)
    Cv = jnp.zeros(nx).at[_tril_rows].add(c * v[_tril_cols])
    y = jnp.dot(Cv, Cv) - 1.
    return y.reshape(-1)

def init_fcn(seed):
    np.random.seed(seed)
    return [np.random.randn(nx, 1), np.random.randn(nx * (nx + 1) // 2)]
...
# Reconstruct matrix P from the optimization vector
x0, c = model.params[:-1]
C = np.zeros((nx, nx))
C[_tril_rows, _tril_cols] = c
P = C.T @ C
print("The inner approximation is the ellipsoid (x-x0)'P(x-x0) <= 1-err, with")
print("P = ", P)
print("x0 = ", x0)
print("err = ", err)
